pipeline {
    ...
    stages {
        stage('config_master'){
            ...
            sh 'sh /usr/lib/jvm/workspace/test_pipe/Spark_zeppelin_script/config_master.sh 15.213.95.216 9092 2 15.213.95.217 15.213.95.218'
            }
        }
        stage('config_slave'){
        parallel{
        ...
        stage('zeppelin_start'){
            agent{label 'spark_master'}
            steps{
                sh 'sh /usr/lib/jvm/workspace/test_pipe/Spark_zeppelin_script/zeppelin_start.sh spark://15.213.95.216:7077'
            }
        }
