pipeline {
    ...
    stages {
        stage('config_master'){
            ...
            sh 'sh /root/scripts/config_master.sh 15.213.95.216 9092 2 15.213.95.217 15.213.95.218'
            }
        }
        stage('config_slave'){
        parallel{
            stage('config_slave1'){
                ...
                sh 'mkdir /root/scripts'    
                sh 'git clone https://github.com/svn123/Spark_zeppelin_script.git /root/scripts'
                sh 'sh /root/scripts/config_slave.sh'
                }
            }
            stage('config_slave2'){
        ...
        stage('start_cluster'){
            agent{label 'spark_master'}
            steps{
                sh 'sh /root/scripts/start_cluster.sh'
            }
        }
